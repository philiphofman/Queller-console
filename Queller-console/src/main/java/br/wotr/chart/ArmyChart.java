...
public class ArmyChart extends Chart {
			...
			while (chartResult == null) {
				...
				if (trunk == 7) {
					...
					if (Main.yes()) {
						...
						if (branch == 2) {
							mu.q("Move would create a new MOBILE army OR increase army size in 1?");
							if (Main.yes()) {
		...
		case 'm':
			...
			while (chartResult == null) {
				...
				if (trunk == 4) {
					...
					if (Main.yes()) {
						mu.q("At least 1 Nation is PASSIVE AND There is an SP army on the route from Barad-dûr to Woodland Realm?");
						if (Main.yes()) {
				...
				if (trunk == 6) {
					...
					if (Main.yes()) {
						...
						if (branch == 2) {
							mu.q("Would create a new MOBILE army OR would increase army size by 1?");
							if (Main.yes()) {
