...
public class MusterChart extends Chart {
	/**
	...
	public String runChart(char strat, String die, int entry) {
		...
		case 'c':
				...
				if (trunk == 2) {
					if (!DiceUtil.savedDiceIsEmpty()) {
						...
						if (Main.yes()) {
							...
							if (Main.yes()) {
								chartResult = mu.a(
										ConstantUtil.getSavemusterdieforlast());
								continue;
							} else {
								fu.printPriorities("MusterChart-Minions");
								fu.printPriorities("CharacterChart-Witch-king");
								fu.printPriorities(
										"CharacterChart-MouthOfSauron");
								chartResult = mu
										.a(ConstantUtil.getMusterminion());
								continue;
							}
						}
					}
				...
				if (trunk == 3) {
					...
					if (Main.yes()) {
						...
						if (Main.isBaseGame()) {
							...
							if (Main.yes()) {
								chartResult = mu
										.a(ConstantUtil.getRecruitfaction());
							} else {
								chartResult = mu
										.a(ConstantUtil.getAdvancenation());
							}
						}
						continue;
					}
			...
			if (chartResult.equals(ConstantUtil.getSavemusterdieforlast())
				...
				if (chartResult
						.equals(ConstantUtil.getSavemusterdieforlast())) {
					DiceUtil.saveDie(dieToUse);
				}
				chartResult = ConstantUtil.getUsed();
			}

			if (chartResult.equals(ConstantUtil.getMuster())
					|| chartResult.equals(ConstantUtil.getMusterminion())
